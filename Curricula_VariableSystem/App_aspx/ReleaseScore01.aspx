<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ReleaseScore01.aspx.cs" Inherits="Curricula_VariableSystem.App_aspx.ReleaseScore01" %>
...
<asp:BulletedList ID="BulletedList1" runat="server" DisplayMode="HyperLink"  CssClass="style2" Height="45px" Width="100%" BorderStyle="NotSet">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:CVSDBConnectionString2 %>" SelectCommand="SELECT Course.课程名称, StudentData.姓名, StudentCourse.学号, Major.年级, Major.专业名称, StudentCourse.成绩 FROM Course INNER JOIN StudentCourse ON Course.课程编号 = StudentCourse.课程编号 INNER JOIN StudentData ON StudentCourse.学号 = StudentData.学号 INNER JOIN Major ON StudentData.专业编号 = Major.专业编号 WHERE (Course.教师工号 = @教师工号) AND (Course.课程名称 = @课程名称)" UpdateCommand="UPDATE StudentCourse SET 成绩 = @成绩 FROM StudentCourse INNER JOIN Course ON StudentCourse.课程编号 = Course.课程编号 WHERE (StudentCourse.学号 = @学号) AND (Course.课程名称 = @课程名称) AND (StudentCourse.课程编号 = StudentCourse.课程编号)">
            <SelectParameters>
                <asp:SessionParameter Name="教师工号" SessionField="Unum" />
                <asp:SessionParameter Name="课程名称" SessionField="Cname" />
            </SelectParameters>
            <UpdateParameters>
                <asp:Parameter Name="成绩" />
                <asp:Parameter Name="学号" />
                <asp:Parameter Name="课程名称" />
            </UpdateParameters>
        </asp:SqlDataSource>
    </form>
</body>
