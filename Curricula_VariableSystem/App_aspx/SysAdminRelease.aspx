<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SysAdminRelease.aspx.cs" Inherits="Curricula_VariableSystem.App_aspx.SysAdminRelease" %>
...
<asp:BulletedList ID="BulletedList1" runat="server" DisplayMode="HyperLink"  CssClass="style2" Height="45px" Width="100%" BorderStyle="NotSet">
        ...
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataKeyNames="课程编号" DataSourceID="SqlDataSource1" Width="100%" >
            <Columns>
                ...
                <asp:TemplateField ShowHeader="False">
                    <ItemTemplate>
                        <asp:LinkButton ID="LinkButton1" runat="server" CausesValidation="False" CommandName="Delete" Text="通过" OnClientClick="return confirm('确定该课程发布通过？');"></asp:LinkButton>
                    </ItemTemplate>

                </asp:TemplateField>
                <asp:TemplateField ShowHeader="False">
                 <ItemTemplate>
                        <asp:LinkButton ID="LinkButton2" runat="server" CausesValidation="False" CommandName="Update" Text="不通过" OnClientClick="return confirm('确定取消该课程？');"></asp:LinkButton>
                    </ItemTemplate>                       
                </asp:TemplateField>
            </Columns>
            <RowStyle HorizontalAlign="Center" />
        </asp:GridView>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:CVSDBConnectionString2 %>" SelectCommand="SELECT Course.课程编号, Course.课程名称, Course.课程类别, Course.学分, Course.上课时间, Course.上课地点, Course.人数上限, Teacher.姓名, Dept.学院名称, Course.课程介绍 FROM Course INNER JOIN Teacher ON Course.教师工号 = Teacher.教师工号 INNER JOIN Dept ON Teacher.学院编号 = Dept.学院编号 WHERE (Course.发布状态 = @发布状态)" DeleteCommand="UPDATE Course SET 发布状态 = 1 WHERE (课程编号 = @课程编号)" UpdateCommand="UPDATE Course SET 发布状态 = 2 WHERE (课程编号 = @课程编号)">
            <DeleteParameters>
                <asp:Parameter Name="课程编号" />
            </DeleteParameters>
            <SelectParameters>
                <asp:Parameter DefaultValue="0" Name="发布状态" />
            </SelectParameters>
            <UpdateParameters>
                <asp:Parameter Name="课程编号" />
            </UpdateParameters>
